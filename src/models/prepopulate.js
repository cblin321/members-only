...
const users = [
    ...
    {
        name: "test 3",
        password: bcrypt.hashSync("890", 10),
...
const insert_template = (table, mapping) => {
    ...
    const values = Object.values(mapping).map(val => typeof val === "string" ? `\'${val}\'` : val).join(", ")
    return `
...
const populate = async () => {
    ...
    try {
        ...
        const user_query = users.map(user => insert_template("users", user)).join("\n")

        console.log(user_query)

        await conn.query(user_query)

        const posts_query = posts.map(post => insert_template("posts", post)).join("\n")
